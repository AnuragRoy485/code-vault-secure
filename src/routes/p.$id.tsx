...
function ViewPaste() {
  ...
  const highlighted = useMemo(() => {
    ...
    try {
      ...
      return escapeHtml(paste.content);
    }
  }, [paste]);
    ...
    <div className="mx-auto max-w-5xl px-4 py-10">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <h1 className="mt-2 font-display text-2xl font-bold tracking-tight sm:text-3xl truncate">
            {paste.title}
          </h1>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-card)]">
        ...
        <div className="grid grid-cols-[auto_1fr]">
          ...
          <pre className="overflow-x-auto py-5 px-5 font-mono text-sm leading-6">
...
function Centered({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="mx-auto max-w-md px-4 py-24 text-center">
      ...
      <Link to="/">
        <Button className="mt-6">Create a paste</Button>
      </Link>
    </div>
  );
}
